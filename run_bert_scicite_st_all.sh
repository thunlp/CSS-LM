...
FINAL_file=final_bert_n_result_open_domain_scicite_entropy_st
RETRIVER_file=retriver_bert_n_result_open_domain_scicite_entropy_st
BASELINE_file=baseline_bert_n_result_open_domain_scicite_entropy_st
###
mkdir $FINAL_file
mkdir $RETRIVER_file
mkdir $BASELINE_file
...
do
    ...
    OUTFILE="output_pretrain_bert_including_Preprocess_DomainTask_sentiment_noaspect_HEADandTAIL_opendomain_scicite_entropy_st"
    ###
    '''
    rm -rf $OUTFILE
    mkdir $OUTFILE
    ...
    do
        ...
        ###
        #OUTFILE="output_pretrain_roberta_including_Preprocess_DomainTask_sentiment_noaspect_HEADandTAIL_opendomain_scicite_entropy_st"
        OUTFILE="output_pretrain_bert_including_Preprocess_DomainTask_sentiment_noaspect_HEADandTAIL_opendomain_scicite_entropy_st_all"
        ###
        rm -rf $OUTFILE
        mkdir $OUTFILE
        ###
        cp -r backup_scicite_all/* $OUTFILE/
        ###
        INPUT=data/openwebtext/train.txt
        OUTPUT=data/opendomain_finetune_scicite_10000_all
        #fix
        ###
        #MODEL=roberta-base-768-scicite-DomainTask-noword-sentiment-HEADandTAIL-opendomain-entropy-self
        MODEL=bert-base-768-scicite-DomainTask-noword-sentiment-HEADandTAIL-opendomain-entropy-self_all
        ###
        mkdir $OUTPUT
        mkdir $MODEL
        rm -rf $MODEL/*
        ...
        RETRIVER_file=retriver_bert_n_result_open_domain_scicite_entropy_st_$K"_"$ITER
        ###
        mkdir $RETRIVER_file
        rm -rf $RETRIVER_file/*
