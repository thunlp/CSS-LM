...
from transformers.modeling_roberta_updateRep import RobertaForMaskedLMDomainTask, RobertaForSequenceClassification
...
model = RobertaForMaskedLMDomainTask.from_pretrained(model, output_hidden_states=False,return_dict=True,num_labels=num_labels,output_attentions=False)
...
with open(file_in) as f:
    ...
    for index, d in tqdm(enumerate(f)):
        ...
        with torch.no_grad():
            ...
            '''
            tail_hidd, head_hidd = model(input_ids_org=torch_ids, attention_mask=attention_mask, func="in_domain_task_rep") #([1,100,768])
