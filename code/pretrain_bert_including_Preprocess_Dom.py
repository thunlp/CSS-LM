...
from transformers.modeling_bert import BertForPreTrainingDomainTask
...
def load_GeneralDomain():
    docs = torch.load("/data5/private/suyusheng/task_selecte/data/open_domain_preprocessed_bert/opendomain_CLS.pt")
    with open("/data5/private/suyusheng/task_selecte/data/open_domain_preprocessed_bert/opendomain.json") as file:
        data = json.load(file)
    return docs, data


docs, data = load_GeneralDomain()


def AugmentationData(top_k, tokenizer, max_seq_length):
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, is_next, lm_label_ids, tail_idxs):
...
def main():
    ...
    # Prepare model
    model = BertForPreTrainingDomainTask.from_pretrained(args.pretrain_model, output_hidden_states=True, return_dict=True)
    model.to(device)
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                '''
                with torch.no_grad():
                    query = model(input_ids=input_ids, masked_lm_labels=lm_label_ids, tail_idxs=tail_idxs, func="query_rep")
                '''
                ###

                in_domain_rep = model(input_ids=input_ids, masked_lm_labels=lm_label_ids, tail_idxs=tail_idxs, func="in_domain_rep")

                # Search id from Docs and ranking: CPU computing
                #.dtype
                query = in_domain_rep.float().to("cpu")
                results = torch.matmul(query, docs.T)
                k=3

                #Domain
                #pos: n ; neg:k*n
                #in_domain_rep_batch = torch.stack([in_domain_rep for i in range(n_gpu)])
                bottom_k = torch.topk(results, k, dim=1, largest=False, sorted=True)
                batch = AugmentationData(bottom_k, tokenizer, args.max_seq_length)
                batch = tuple(t.to(device) for t in batch)
                input_ids, input_mask, segment_ids, lm_label_ids, is_next, tail_idxs = batch
                domain_loss = model(input_ids=input_ids, masked_lm_labels=lm_label_ids, func="domain_class", in_domain_rep=in_domain_rep.to(device))

                #Task
                top_k = torch.topk(results, k, dim=1, largest=True, sorted=True)
                batch = AugmentationData(top_k, tokenizer, args.max_seq_length)
                batch = tuple(t.to(device) for t in batch)
                input_ids, input_mask, segment_ids, lm_label_ids, is_next, tail_idxs = batch
                output_pos = model(input_ids=input_ids, masked_lm_labels=lm_label_ids, func=None)
