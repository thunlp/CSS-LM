...
def load_outdomain(data_dir_outdomain, tokenizer, max_seq_length):
    ###Open
    doc_line = list()
    with open(data_dir_outdomain+"test.json") as file:
    ...
    for line in all_data:
        domain_list_all.append(line["domain"])
        task_list_all.append(line["sentiment"])

    domain_list_all = sorted(list(set(domain_list_all)))
    domain_map_id = {domain:id for id,domain in enumerate(domain_list_all)}
    print(domain_map_id)


    task_list_all = sorted(list(set(task_list_all)))
    task_map_id = {task:id for id,task in enumerate(task_list_all)}
    print(task_map_id)
    ...
    for i, data in enumerate(all_data):
        ...
        cur_tensors = (torch.tensor(cur_features.input_ids),
                       ...
                       torch.tensor(0),
                       torch.tensor(cur_features.tail_idxs),
                       torch.tensor(domain_map_id[domain]),
                       torch.tensor(task_map_id[task]))

        cur_tensors_list.append(cur_tensors)

    return cur_tensors_list, domain_map_id, task_map_id
...
def main():
    ...
    if True:
        ...
        train_dataset, domain_map_id, task_map_id = load_outdomain(args.data_dir_outdomain, tokenizer, args.max_seq_length)
    ...
    for step, batch_ in enumerate(tqdm(train_dataloader, desc="Iteration")):
        ...
        input_ids_, input_ids_org_, input_mask_, segment_ids_, lm_label_ids_, is_next_, tail_idxs_, domain_label_, task_label_ = batch_
        ...
        with torch.no_grad():
            ...
            if step==0:
                ...
                docs_domain.append(domain_label_)
                docs_task.append(task_label_)
            else:
                if docs_tail[-1].shape[0] == in_domain_rep.shape[0]:
                    ...
                    rest_domain = domain_label_
                    rest_task = task_label_
    ...
    docs_domain = torch.stack(docs_domain)
    docs_domain = docs_domain.reshape(docs_domain.shape[0]*docs_domain.shape[1])
    rest_domain = rest_domain.reshape(rest_domain.shape[0])
    docs_domain = torch.cat([docs_domain,rest_domain],0)


    docs_task = torch.stack(docs_task)
    docs_task = docs_task.reshape(docs_task.shape[0]*docs_task.shape[1])
    rest_task = rest_task.reshape(rest_task.shape[0])
    docs_task = torch.cat([docs_task,rest_task],0)


    docs_tail = docs_tail.to("cpu")
    docs_head = docs_head.to("cpu")
    docs_domain = docs_domain.to("cpu")
    docs_task = docs_task.to("cpu")
    ...
    total_score_task = torch.zeros([args.num_labels_task,docs_head.shape[0]]).to("cpu")
    #total_score_domain = torch.zeros([1,docs_head.shape[0]]).to("cpu")
    total_score_domain = torch.zeros([args.num_labels_task,docs_head.shape[0]]).to("cpu")
    if True:
    ...
    for step, batch_ in enumerate(tqdm(train_dataloader, desc="Iteration")):
        ...
        #################
        #################
        # 3 type label  -->check label every time and add corresponding rank_list
        ...
        for i_th, task in enumerate(sentiment_label_):
            total_score_task[task] += task_binary_logit[i_th]
            total_score_domain[task] += domain_binary_logit[i_th]

        '''
        for i_th, domain in enumerate(domain_label_):
            total_score_domain[0] += domain_binary_logit[i_th]
        '''
    ...
    total_score = total_score_task + total_score_domain

    ########
    ########
    print("===========")
    print("Domain:",domain_map_id)
    print("-----------")
    print("Task:",task_map_id)
    print("===========")

    ########
    '''
    print(total_score_domain.shape)
    print(total_score_task.shape)
    total_score = total_score.sum(0).unsqueeze(0)
    print(total_score.shape)
    '''
    ########
    ...
    #Ranking
    ###Domain
    total_score_domain_ = total_score_domain.mean(0).unsqueeze(0)
    top_k_domain = torch.topk(total_score_domain_, k, dim=1, largest=True, sorted=False)
    top_k_domain = {"values":top_k_domain.values, "indices":top_k_domain.indices}
    ###Task
    top_k_task = torch.topk(total_score_task, k, dim=1, largest=True, sorted=False)
    top_k_task = {"values":top_k_task.values, "indices":top_k_task.indices}
    ###Both
    top_k = torch.topk(total_score, k, dim=1, largest=True, sorted=False)
    top_k = {"values":top_k.values, "indices":top_k.indices}


    ###Domain
    #[1,]
    choosed_docs_domain = top_k_domain["indices"]
    ###Task
    #[3,]
    choosed_docs_task = top_k_task["indices"]
    ###Both
    #[3,]
    choosed_docs = top_k["indices"]
    ...
    #######Domain#########
    #docs_task, all doc --> type:0,1,2,3,..
    #docs_domain, all doc --> type:0,1
    #[type, top_n]
    ###Type
    #R, P, F
    #Task: 128*2 / 128*6
    #Domain: 128*3 / 128*6
    #All type and domain: 128*1 / 128*6
    ...
    ###domain
    in_domain_counter_all = 0
    for type_id, task_type_ranking_id in enumerate(choosed_docs_domain): #1
        in_domain_counter = 0
        for id in task_type_ranking_id:
            #if docs_domain[id] == 1:
            if docs_domain[id] == 1:
                in_domain_counter+=1
                in_domain_counter_all+=1
        #in_domain[type_id] = in_domain_counter
    in_domain["all"] = in_domain_counter_all

    in_domain_counter_all_mix = 0
    for type_id, task_type_ranking_id in enumerate(choosed_docs): #3
        in_domain_counter_mix = 0
        for id in task_type_ranking_id:
            #if docs_domain[id] == 1:
            if docs_domain[id] == 1:
                in_domain_counter_mix+=1
                in_domain_counter_all_mix+=1
        #in_domain[type_id] = in_domain_counter
    in_domain_mix["all"] = in_domain_counter_all_mix

    ###task
    in_task_counter_all = 0
    for type_id, task_type_ranking_id in enumerate(choosed_docs_task): #3
        in_task_counter = 0
        for id in task_type_ranking_id:
            #if docs_task[id] == type_id:
            if docs_task[id] == type_id:
                in_task_counter+=1
                in_task_counter_all+=1
        in_task[type_id] = in_task_counter
    in_task["all"] = in_task_counter_all

    in_task_counter_all_mix = 0
    for type_id, task_type_ranking_id in enumerate(choosed_docs): #3
        in_task_counter_mix = 0
        for id in task_type_ranking_id:
            #if docs_task[id] == type_id:
            if docs_task[id] == type_id:
                in_task_counter_mix+=1
                in_task_counter_all_mix+=1
        in_task[type_id] = in_task_counter
    in_task_mix["all"] = in_task_counter_all_mix
    ...
    print("+++++++++")
    print("Recall@",k)
    for key,val in in_domain.items():
        if key == "all":
            try:
                score = 100*val/128
            except:
                score = 0
            print("Label_{}: {}%".format(key,score))
        #else:
        #    print("Label_{}: {}".format(key,val/(128*3)))
            try:
                RP["r"] = val/(128)
            except:
                RP["r"] = 0
    ...
    print("------------")
    print("F1@",k)
    #print("Label_all: {}%".format( 100*2*RP["p"]*RP["r"]/(RP["p"]+RP["r"]) ))
    try:
        print("Label_all: {}%".format( 100*2*RP["p"]*RP["r"]/(RP["p"]+RP["r"]) ))
    except:
        print("Label_all: {}%".format(0) )
    ...
    for key,val in in_task.items():
        if key == "all":
            try:
                ...
                score = 0
            print("Label_{}: {}%".format(key,score))
            #print("Label_{}: {}%".format(key,100*val/(k*3)))
        #else:
        #    print("Label_{}: {}".format(key,val/k))
            try:
                RP["p"] = val/(k*3)
    ...
    for type_id, task_type_ranking_id in enumerate(choosed_docs_domain): #1
        ...
        for id in task_type_ranking_id:
            #if docs_domain[id] == 1:
            if docs_task[id] == type_id and docs_domain[id] == 1:
                in_domain_counter+=1
                in_domain_counter_all+=1
        #in_domain[type_id] = in_domain_counter
    in_domain["all"] = in_domain_counter_all
    ...
    ###domain task
    in_task_domain_counter_all = 0
    for type_id, task_type_ranking_id in enumerate(choosed_docs): #3
        in_task_domain_counter = 0
        for id in task_type_ranking_id:
            if docs_task[id] == type_id and docs_domain[id] == 1:
                in_task_domain_counter+=1
                in_task_domain_counter_all+=1
        in_task_domain[type_id] = in_task_domain_counter
    in_task_domain["all"] = in_task_domain_counter_all
    ...
    print("Domain:",in_domain)
    print("Task:",in_task)
    print("Domain and Task",in_task_domain)
    ...
    for key,val in in_domain.items():
        if key == "all":
            try:
                ...
                score = 0
            print("Label_{}: {}%".format(key,score))
            #print("Label_{}: {}%".format(key,100*val/(128)))
        #else:
        #    print("Label_{}: {}".format(key,val/(128*3)))
            try:
                RP["r"] = val/(128)
    ...
    for key,val in in_domain.items():
        if key == "all":
            try:
                ...
                score = 0
            print("Label_{}: {}%".format(key,score))
            #print("Label_{}: {}%".format(key,100*val/k))
        #else:
        #    print("Label_{}: {}".format(key,val/k))
            try:
                RP["p"] = val/k
    ...
    for key,val in in_task.items():
        if key == "all":
            try:
                ...
                score = 0
            print("Label_{}: {}%".format(key,score))
            #print("Label_{}: {}%".format(key,100*val/(128*3)))
        #else:
        #    print("Label_{}: {}".format(key,val/(128*3)))
            try:
                RP["r"] = val/(128*3)
    ...
    print("================================")
    print("+++++++++")
    print("Domain and Task")
    RP = dict()
    print("+++++++++")
    print("Recall@",k)
    for key,val in in_task_domain.items():
        if key == "all":
            try:
                score = 100*val/(128*3)
            except:
