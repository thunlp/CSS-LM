...
def main():
    ...
    # Prepare model
    #model, _ = BertForSequenceClassification.from_pretrained(args.ernie_model,cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(args.local_rank),num_labels = num_labels, args=args)

    #model = BertForPreTraining.from_pretrained(args.pretrain_model, output_hidden_states=True,return_dict=True)

    #Looking for how many parameters here!!
    #model = BertForSequenceClassification.from_pretrained(args.pretrain_model, num_labels=num_labels, output_hidden_states=True, output_attentions=True, return_dict=True)
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if args.task == 1:
                    ...
                    #output = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=attention_mask, labels=label_ids)
                    output = model(input_ids=input_ids, token_type_ids=None, attention_mask=None, labels=label_ids)
                    loss = output.loss
                else:
                    print("Wrong!!")
