...
from transformers.modeling_roberta_updateRep import RobertaForSequenceClassification
...
model = GPT2Model.from_pretrained('gpt2')
        ...
        #<pad>:1
        padding = [0] * (max_seq_length - len(input_ids))
        paddingid = [1] * (max_seq_length - len(input_ids))
        input_ids += paddin_id
...
def main():
    ...
    eval_examples, aspect_list, sentiment_list = processor.get_dev_examples(args.data_dir)
    if args.task == 1:
    ...
    for x, mark in file_mark:
        ...
        #model = RobertaForMaskedLMDomainTask.from_pretrained(args.pretrain_model, num_labels=num_labels, output_hidden_states=False, output_attentions=False, return_dict=True)
