...
class InputFeatures(object):
    ...
    def __init__(self, input_ids=None, attention_mask=None, segment_ids=None, label_id=None, aspect_id=None):
...
def convert_examples_to_features(examples, aspect_list, sentiment_list, max_seq_length, tokenizer, task_n):
    ...
    aspect_label_list = sorted(aspect_list)

    label_map = {label : i for i, label in enumerate(label_list)}
    aspect_label_map = {label : i for i, label in enumerate(aspect_label_list)}
    print("=======")
    print(label_map)
...
model = GPT2Model.from_pretrained('gpt2')
        ...
        aspect_id = aspect_label_map[example.aspect]
...
def main():
    ...
    for x, mark in file_mark:
        ...
        if args.task == 1:
            eval_data = TensorDataset(all_input_ids, all_attention_mask, all_label_ids, all_aspect_ids)
        elif args.task == 2:
            eval_data = TensorDataset(all_input_ids, all_attention_mask, all_segment_ids, all_label_ids, all_aspect_ids)
        else:
            print("Wrong here1")
