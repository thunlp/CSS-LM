...
#docs_head, docs_tail, data = load_GeneralDomain("data/yelp_finetune_noword_10000/")
parser = argparse.ArgumentParser()
parser = get_parameter(parser)
args = parser.parse_args()
#print(args.data_dir_outdomain)
docs_head, docs_tail, data = load_GeneralDomain(args.data_dir_outdomain)
...
def in_Domain_Task_Data_mutiple(data_dir_indomain, tokenizer, max_seq_length):
    ...
    for line in data:
        ...
        label = line["label"]
        rest = line["h"][0]+" <pad> "+line["t"][0]+" <pad>"

        rest = tokenizer.tokenize(rest)
        ...
        # tokenize
        cur_example = InputExample(guid=id, tokens_a=tokens_a, tokens_b=None, is_next=0, rest=rest)
        ...
        cur_tensors = (torch.tensor(cur_features.input_ids),
                       ...
                       torch.tensor(cur_features.input_org_mask),
    ...
    return all_type_sentiment_sentence, cur_tensors_list, label_map
...
def in_Domain_Task_Data_binary(data_dir_indomain, tokenizer, max_seq_length):
    ...
    for line in data:
        ...
        try:
            ...
            label_sentence_dict[line["label"]].append([line["tokens"]])
        except:
            #label_sentence_dict[line["aspect"]] = [line["sentence"]]
            #label_sentence_dict[line["label"]] = [line["sentence"]]
            label_sentence_dict[line["label"]] = [line["tokens"]]

    num_label = sorted(list(set(num_label_list)))
    label_map = {label : i for i , label in enumerate(num_label)}
    id_label_map = {val:key for key, val in label_map.items()}
    ...
    for line in data:
        ...
        for id, sentence_label in enumerate(all_sentence):
            ...
            rest = id_label_map[sentence_label_[id_1]]+" <pad> "+id_label_map[sentence_label_[id_1]]+" <pad>"
            rest = tokenizer.tokenize(rest)
            tokens_a = tokenizer.tokenize(sentence_label[0])
            '''
            if "</s>" in tokens_a:
            ...
            all_input_org_mask.append(torch.tensor(cur_features.input_org_mask))
        ...
        cur_tensors = (torch.stack(all_input_ids),
                       ...
                       torch.stack(all_input_org_mask),
...
def AugmentationData_Task(top_k, tokenizer, max_seq_length, add_org=None, label_map=None):
    ...
    input_ids_, input_ids_org_, input_mask_, input_org_mask_, segment_ids_, lm_label_ids_, is_next_, tail_idxs_, sentence_label_, sentiment_label_ = add_org
    ...
    for id_1, sent in enumerate(sentence_ids):
        for id_2, sent_id in enumerate(sent):
            ...
            rest = label_map[int(sentence_label_[id_1])]+" <pad> "+label_map[int(sentence_label_[id_1])]+" <pad>"
            rest = tokenizer.tokenize(rest)
            tokens_a = tokenizer.tokenize(t1)
        ...
        all_input_org_mask.append(input_org_mask_[id_1])
...
def AugmentationData_Task_pos_and_neg(top_k=None, tokenizer=None, max_seq_length=None, add_org=None, in_task_rep=None):
    ...
    #input_ids_, input_ids_org_, input_mask_, segment_ids_, lm_label_ids_, is_next_, tail_idxs_, sentence_label_ = add_org
    input_ids_, input_ids_org_, input_mask_, input_org_mask, segment_ids_, lm_label_ids_, is_next_, tail_idxs_, sentence_label_, sentiment_label_ = add_org
...
class InputExample(object):
    ...
    def __init__(self, guid, tokens_a, tokens_b=None, is_next=None, lm_labels=None, rest=None):
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_ids_org, input_mask, input_org_mask, segment_ids, is_next, lm_label_ids, tail_idxs):
        self.input_ids = input_ids
        self.input_ids_org = input_ids_org
        self.input_mask = input_mask
        self.input_org_mask = input_org_mask
...
def convert_example_to_features(example, max_seq_length, tokenizer):
    ...
    try:
        rest = tokenizer.encode(rest, add_special_tokens=False)
        if len(input_ids_org+rest) <= max_seq_length:
            input_ids_org += rest
        elif len(input_ids_org) == max_seq_length:
            input_ids_org = input_ids_org[:len(input_ids_org)-len(rest)-1]+[2]+rest
        else:
            input_ids_org = input_ids_org[:max_seq_length]
            input_ids_org = input_ids_org[:len(input_ids_org)-len(rest)-1]+[2]+rest
    except:
        pass
    ...
    input_org_mask = [1] * len(input_ids)
    ...
    while len(input_ids_org) < max_seq_length:
        input_ids_org.append(pad_id)
        input_org_mask.append(0)
    ...
    assert len(input_org_mask) == max_seq_length
...
def main():
    ...
    if args.do_train:
        ...
        all_type_sentence, train_dataset, label_map = in_Domain_Task_Data_mutiple(args.data_dir_indomain, tokenizer, args.max_seq_length)
        ...
        for epo in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch_ in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if (step == 0) and (epo == 0):
                    ...
                    input_org_mask_ = torch.stack([line[3] for line in all_type_sentence]).to(device)
                    segment_ids_ = torch.stack([line[4] for line in all_type_sentence]).to(device)
                    lm_label_ids_ = torch.stack([line[5] for line in all_type_sentence]).to(device)
                    is_next_ = torch.stack([line[6] for line in all_type_sentence]).to(device)
                    tail_idxs_ = torch.stack([line[7] for line in all_type_sentence]).to(device)
                    sentence_label_ = torch.stack([line[8] for line in all_type_sentence]).to(device)
                    sentiment_label_ = torch.stack([line[9] for line in all_type_sentence]).to(device)
                    ...
                    with torch.no_grad():
                        ...
                        #in_domain_rep, in_task_rep = model(input_ids_org=input_ids_org_, tail_idxs=tail_idxs_, attention_mask=input_mask_, func="in_domain_task_rep")
                        in_domain_rep, in_task_rep = model(input_ids_org=input_ids_org_, tail_idxs=tail_idxs_, attention_mask=input_org_mask_, func="in_domain_task_rep")
                        # Search id from Docs and ranking via (Domain/Task)
                ...
                ###Normal mode
                batch_ = tuple(t.to(device) for t in batch_)
                input_ids_, input_ids_org_, input_mask_, input_org_mask_, segment_ids_, lm_label_ids_, is_next_, tail_idxs_, sentence_label_, sentiment_label_ = batch_
                ...
                input_ids, input_ids_org, input_mask, input_org_mask, segment_ids, lm_label_ids, is_next, tail_idxs = batch
                #domain_binary_loss, domain_binary_logit, out_domain_rep_head, out_domain_rep_tail = model(input_ids_org=input_ids_org, lm_label=lm_label_ids, attention_mask=input_mask, func="domain_binary_classifier", in_domain_rep=in_domain_rep.to(device))
                domain_binary_loss, domain_binary_logit, out_domain_rep_head, out_domain_rep_tail = model(input_ids_org=input_ids_org, lm_label=lm_label_ids, attention_mask=input_org_mask, func="domain_binary_classifier", in_domain_rep=in_domain_rep.to(device))

                #################
                #################
                ###Update_rep
                indices = bottom_k["indices"].reshape(bottom_k["indices"].shape[0]*bottom_k["indices"].shape[1])
                ...
                '''
                print("===========")
                print("bottom_k_indices",bottom_k["indices"].shape)
                print("bottom_k_values",bottom_k["values"].shape)
                print("input_ids_org",input_ids_org.shape)
                print("head",out_domain_rep_head.shape)
                print("tail",out_domain_rep_head.shape)
                print("loss",domain_binary_loss.shape)
                ...
                task_loss_org, class_logit_org = model(input_ids_org=input_ids_org_, sentence_label=sentiment_label_, attention_mask=input_org_mask_, func="task_class")

                #################
                #################
                #pre-train org
                #masked_loss_org = model(input_ids=input_ids_, lm_label=lm_label_ids_, attention_mask=input_mask_, func="mlm")

                #if epo > 2:
                batch = AugmentationData_Task(top_k, tokenizer, args.max_seq_length, add_org=batch_, label_map=label_map)
                batch = tuple(t.to(device) for t in batch)
                input_ids, input_ids_org, input_mask, input_org_mask, segment_ids, lm_label_ids, is_next, tail_idxs, sentence_label, sentiment_label = batch

                #################
                #################
                ###Train task query - finetune
                #task_loss_query, class_logit_query, out_domain_rep_head, out_domain_rep_tail = model(input_ids_org=input_ids_org, sentence_label=sentiment_label, attention_mask=input_mask, func="task_class", from_query=True)
                task_loss_query, class_logit_query, out_domain_rep_head, out_domain_rep_tail = model(input_ids_org=input_ids_org, sentence_label=sentiment_label, attention_mask=input_org_mask, func="task_class", from_query=True)

                #################
                #################
                ###pre-train query
                #masked_loss_query = model(input_ids=input_ids, lm_label=lm_label_ids, attention_mask=input_mask, func="mlm")


                #################
                #################
                ###Update_rep
                indices = top_k["indices"].reshape(top_k["indices"].shape[0]*top_k["indices"].shape[1])
                ...
                #else:
                #    task_loss_query = torch.tensor([0.0]).to(device)
                #    masked_loss_query = torch.tensor([0.0]).to(device)
                ...
                if n_gpu > 1:
                    ...
                    loss = domain_binary_loss + task_binary_loss + task_loss_org + pseudo
