...
class RobertaForMaskedLMDomainTask(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.layer_out_taskClass = nn.Linear(768,8) #num_class
        ###
        self.domain_binary_classifier = nn.Linear(768*2,2) #num_class
        self.task_binary_classifier = nn.Linear(768*2,8) #num_class
        ###
    ...
    def forward(
        ...
        **kwargs
        ...
        '''
        if func == "return domain_layer":
            return self.domain_layer

        if func == "return task_layer":
            return self.task_layer
        '''

        if func == "domain_class":
            #######
            outputs = self.roberta(
                input_ids=input_ids_org,
                attention_mask=attention_mask,
            ...
            '''
            #out_domain_rep = outputs.last_hidden_state[input_ids==2]
            out_domain_rep = outputs.last_hidden_state[:, 0, :]
            #detach
            out_domain_rep = out_domain_rep.detach()
            out_domain_rep = self.domain_layer(out_domain_rep)
            domain_rep = torch.cat([in_domain_rep, out_domain_rep], 0)
            logit = self.layer_out_domainClass((self.act(domain_rep)))
        ...
        '''
        if func == "return task_binary_classifier":
            print(self.task_binary_classifier)
            print(type(self.task_binary_classifier))
            exit()
            return self.task_binary_classifier
        '''

        '''
        if func == "return domain_binary_classifier":
            return self.domain_binary_classifier
        '''
