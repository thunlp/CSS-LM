...
class RobertaForMaskedLMDomainTask(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.classifier_Task = RobertaClassificationHead(config)
        self.classifier_Domain = RobertaClassificationTail(config)
        self.classifier_DomainandTask = RobertaClassificationHeadandTail(config)
        ...
        #self.ratio = self.softmax(nn.Linear(2,2),bias=False)
        #self.ratio = self.sigmoid(torch.FloatTensor(1)).to("cuda")
        #self.act = nn.ReLU()
        ###
...
class RobertaClassificationTail(nn.Module):
    """Head for sentence-level classification tasks."""
