...
#outdomain = torch.load("/data5/private/suyusheng/task_selecte/data/opendomain_CLS.pt")
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids=None, input_mask=None, masked_lm_positions=None, masked_lm_labels=None):
    #def __init__(self, input_ids=None, masked_lm_positions=None, masked_lm_labels=None):
...
class InputExample(object):
    ...
    def __init__(self, guid, input_ids, input_mask, masked_lm_positions, masked_lm_labels):
...
class Processor_pretrain(DataProcessor):
    """Processor for the CoLA data set (GLUE version)."""

    def get_train_examples(self, data_dir):
        """See base class."""
        examples = self._create_examples(
            self._read_json(os.path.join(data_dir, "pretrain.json")), "train")
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            #forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None, next_sentence_label=None)
            #Pretrain (Old version)
            #model(input_ids, segment_ids, input_mask, lm_label_ids, is_next)

            #Needed Parameters: input_ids, attention_mask, masked_lm_positions, masked_lm_labels
            #Need to be fix: input_ids, attention_mask, masked_lm_positions, masked_lm_labels

            input_ids = line["input_ids"]
            input_mask = line["input_mask"]
            #segment_ids = line["segment_ids"]
            token_type_ids = line["segment_ids"]
            masked_lm_positions = line["masked_lm_positions"]
            masked_lm_ids = line["masked_lm_ids"]
            masked_lm_weights = line["masked_lm_weights"]
            next_sentence_labels = line["next_sentence_labels"]

            examples.append(
                InputExample(guid=guid, input_ids=input_ids, input_mask=input_mask, masked_lm_positions=masked_lm_positions, masked_lm_labels=masked_lm_ids))
        return examples
...
def convert_examples_to_features(examples, aspect_list, sentiment_list, max_seq_length, tokenizer, task_n):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        idx = example.input_ids.index(102) #the end of the 1st sentence
        input_ids = example.input_ids[:idx+1] + [0]*(max_seq_length-idx-1)
        input_mask = example.input_mask[:idx+1] + [0]*(max_seq_length-idx-1)
        masked_lm_labels = [-1]*max_seq_length
        for index, pos in enumerate(example.masked_lm_positions):
            if pos==0:
                continue
            else:
                masked_lm_labels[pos] = example.masked_lm_positions[index]

        #masked_lm_positions = [num if num<=idx else 0 for num in example.masked_lm_positions]
        #idx_ = masked_lm_positions.index(0)
        #masked_lm_labels = example.masked_lm_labels[:idx_] + [0]*(len(example.masked_lm_labels)-idx_)
...
def main():
    ...
    # Prepare model
    model =  BertForPreTraining.from_pretrained(args.pretrain_model, return_dict=True)
    ...
    if args.do_train:
        ...
        train_features = convert_examples_to_features( train_examples, aspect_list=None, sentiment_list=None, max_seq_length=args.max_seq_length, tokenizer=tokenizer, task_n=args.task)
        ...
        if args.task == 1:
            ...
            train_data = TensorDataset(all_input_ids, all_input_mask, all_masked_lm_labels)
        else:
            print("Wrong here1")
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if args.task == 1:
                    ...
                    output = model(input_ids=input_ids, token_type_ids=None, input_mask=input_mask, labels=None)
                    loss = output.loss
                    #loss=<generator object gather.<locals>.gather_map.<locals>.<genexpr> at 0x7f52ec67f8e0>
                elif args.task == 2:
                    #loss, logits, hidden_states, attentions
                    output = model(input_ids=input_ids, token_type_ids=segment_ids, input_mask=input_mask, labels=None)
                    loss = output.loss
                elif args.task == 0:
                    #loss, logits, hidden_states, attentions
                    #output = model(input_ids=input_ids, input_mask=input_mask, position_ids=masked_lm_positions, labels=masked_lm_labels)
                    output = model(input_ids=input_ids, attention_mask=None, position_ids=None, masked_lm_labels=masked_lm_labels)
                    loss = output.loss
                else:
                    print("Wrong!!")
