...
class BERTDataset_noNext(Dataset):
...
class BERTDataset(Dataset):
    def __init__(self, corpus_path, tokenizer, seq_len, encoding="utf-8", corpus_lines=None, on_memory=True):
        ...
        if on_memory:
            ...
            doc = []
            self.corpus_lines = 0
            #print(os.path.join(corpus_path))
            with open(corpus_path, "r", encoding=encoding) as f:
            #with open(os.path.join(corpus_path), "r", encoding=encoding) as f:
                for line in tqdm(f, desc="Loading Dataset", total=corpus_lines):
                    line = line.strip()
                    if line == "":
                        self.all_docs.append(doc)
...
def main():
    ...
    if args.do_train:
        ...
        train_dataset = BERTDataset_noNext(args.data_dir, tokenizer, seq_len=args.max_seq_length,
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                #loss = model(input_ids=input_ids, segment_ids, input_mask, lm_label_ids, is_next)
