...
def in_Domain_Task_Data_mutiple(data_dir_indomain, tokenizer, max_seq_length):
    ...
    ###Preprocess
    num_label_list = list()
    num_sentiment_list = list()
    label_sentence_dict = dict()
    for line in data:
        #line["sentence"]
        #line["aspect"]
        #line["sentiment"]
        num_sentiment_list.append(line["sentiment"])
    ...
    print(label_map)
    print("=======")
    num_sentiment_label = sorted(list(set(num_sentiment_list)))
    label_sentiment_map = {label : i for i , label in enumerate(num_sentiment_label)}
    print("=======")
    print("label_sentiment_map:")
    print(label_sentiment_map)
    print("=======")
    ...
    candidate_sentiment_list = list(label_sentiment_map.values())

    #all_type_sentence = [0]*len(candidate_label_list)
    #all_type_sentence_sentiment = [0]*len(candidate_label_list)*len(candidate_sentiment_list)
    all_type_sentence_sentiment = list()
    for i in candidate_label_list:
        all_type_sentence_sentiment.append(candidate_sentiment_list)
    #all_type_sentence_sentiment = [0]*len(candidate_label_list)
    for line in data:
        #line["sentence"]
        #line["aspect"]
        #line["sentiment"]
        ...
        cur_features = convert_example_to_features(cur_example, max_seq_length, tokenizer, label)
        ...
        cur_tensors = (torch.tensor(cur_features.input_ids),
                       ...
                       torch.tensor(label_sentiment_map[sentiment])
                       )
        ...
        '''
        all_type_sentence_sentiment[int(label_map[label])][int(label_sentiment_map[sentiment])] = [cur_tensors]
        #all_type_sentence_sentiment[int(label_map[label])][int(label_sentiment_map[sentiment])] = cur_tensors
        #print(all_type_sentence_sentiment)
        ###
    ...
    #return cur_tensors
    #return all_type_sentence, cur_tensors_list, label_map, label_sentiment_map

    all_type_sentence_sentiment = [j for i in all_type_sentence_sentiment for j in i]
    #print(all_type_sentence_sentiment)
    #exit()

    return all_type_sentence_sentiment, cur_tensors_list, label_map, label_sentiment_map
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_ids_org, input_mask, segment_ids, is_next, lm_label_ids, tail_idxs, input_mask_org, unk_idxs):
        ...
        self.input_mask_org = input_mask_org
        self.unk_idxs = unk_idxs
...
def convert_example_to_features(example, max_seq_length, tokenizer, aspect_word):
    """
    ...
    aspect_token = tokenizer.encode(aspect_word, add_special_tokens=False)
    #print(tokenizer.convert_ids_to_tokens(3))
    #print(tokenizer.encode("<unk>")) #3
    aspect_token = [3] + aspect_token + [2]
    # Modifies `tokens_a` and `tokens_b` in place so that the total
    # length is less than the specified length.
    # Account for [CLS], [SEP], [SEP] with "- 3"
    #_truncate_seq_pair(tokens_a, tokens_b, max_seq_length - 3)
    #_truncate_seq_pair(tokens_a, tokens_b, max_seq_length - 2)
    _truncate_seq_pair(tokens_a, tokens_b, max_seq_length - 2 - len(aspect_token))
    ...
    input_mask_org = [1] * len(input_ids_org)
    ...
    assert len(input_mask_org) == max_seq_length
...
def main():
    ...
    if args.do_train:
        ...
        all_type_sentence, train_dataset, label_map, label_sentiment_map = in_Domain_Task_Data_mutiple(args.data_dir_indomain, tokenizer, args.max_seq_length)
    ...
    if args.do_train:
        ...
        for epo in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch_ in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                #input_ids_, input_ids_org_, input_mask_, segment_ids_, lm_label_ids_, is_next_, tail_idxs_, sentence_label_, sentence_sentiment_label_ = batch_
                ...
                if (step == 0) and (epo == 0):
                    ...
                    sentence_sentiment_label_ = torch.stack([line[8] for line in all_type_sentence]).to(device)

                    print(sentence_label_)
                    print(sentence_sentiment_label_)
                    exit()
