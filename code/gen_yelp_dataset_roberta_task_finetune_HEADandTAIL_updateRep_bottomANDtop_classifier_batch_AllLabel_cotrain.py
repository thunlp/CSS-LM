...
def in_Domain_Task_Data_binary(data_dir_indomain, tokenizer, max_seq_length):
    ...
    for line in data:
        ...
        try:
            label_sentence_dict[line["sentiment"]].append([line["sentence"]])
        except:
            label_sentence_dict[line["sentiment"]] = [line["sentence"]]
...
def main():
    ...
    if int(args.K) > int(docs_head.shape[0]):
        top_k_num = int(int(docs_head.shape[0])/(2*args.num_labels_task))
        bottom_k_num = top_k_num
    else:
        print("Here")
        top_k_num = int(int(args.K)/(2*args.num_labels_task))
        bottom_k_num = top_k_num
    ...
    with torch.no_grad():
        ...
        for step, batch_ in enumerate(tqdm(train_dataloader, desc="Iteration")):
            ...
            domain_binary_logit = domain_binary_classifier(torch.cat([query_domain, docs_head[:,0,:].unsqueeze(0).expand(sentiment_label_.shape[0], -1, -1)], dim=2))
            ...
            '''
            for index_batch, type in enumerate(sentiment_label_):
                total_score_task[type] += task_binary_logit[index_batch]
                total_score_domain[type] += domain_binary_logit[index_batch]
            '''
            for type in range(args.num_labels_task):
                total_score_task[type] += task_binary_logit[sentiment_label_==type].sum(dim=0)
                total_score_domain[type] += domain_binary_logit[sentiment_label_==type].sum(dim=0)
    ...
    #[type,]
    total_score = total_score_domain+total_score_task

    #Ranking
    #bottom_k = torch.topk(total_score, k, dim=0, largest=False, sorted=False)
    bottom_k = torch.topk(total_score, bottom_k_num, dim=1, largest=False, sorted=False)
    bottom_k = {"values":bottom_k.values, "indices":bottom_k.indices}
    #top_k = torch.topk(total_score, k, dim=0, largest=True, sorted=False)
    top_k = torch.topk(total_score, top_k_num, dim=1, largest=True, sorted=False)
    ...
    print("====")
    print(bottom_k["indices"])
    print(top_k["indices"])
    print("====")
    print(bottom_k["indices"].reshape(bottom_k["indices"].shape[0]*bottom_k["indices"].shape[1]))
    print(top_k["indices"].reshape(top_k["indices"].shape[0]*top_k["indices"].shape[1]))
    '''
    ###

    bottom_k["indices"] = bottom_k["indices"].reshape(bottom_k["indices"].shape[0]*bottom_k["indices"].shape[1])
    top_k["indices"] = top_k["indices"].reshape(top_k["indices"].shape[0]*top_k["indices"].shape[1])
    ...
    for id, index in enumerate(choosed_docs):
        ...
        all_data_dict[id] = {"sentence":tokenizer.decode(id_doc_tensor[index]).replace("<pad>","").replace("<s>","").replace("</s>","").replace("\n","").replace("\u0000","")}
