            ...
            for step, batch_ in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if (step == 0) and (epo == 0):
                    ...
                    with torch.no_grad():
                        ...
                        results_all_type = task_binary_logit
                        del task_binary_logit
                ...
                #task_loss_org, class_logit_org = model(input_ids_org=input_ids_org_, sentence_label=sentiment_label_, attention_mask=input_mask_, func="task_class_nodomain")
                ...
                #task_loss_cotrain, class_logit_cotrain, out_domain_rep_head, out_domain_rep_tail = model(input_ids_org=input_ids_org, sentence_label=sentiment_label, attention_mask=input_mask, func="task_class_nodomain", from_query=True)
                #####MLM Task
                mlm_loss = model(input_ids=input_ids_, lm_label=lm_label_ids_, attention_mask=input_mask_, func="mlm")
                #####
                ###
                ...
                if n_gpu > 1:
                    ...
                    loss = task_binary_loss.mean() + task_loss_org.mean() + pseudo + mlm_loss.mean() #+ task_loss_cotrain.mean()*task_loss_cotrain.mean()/(task_loss_cotrain.mean()+task_loss_org.mean())
                    ...
                    loss = task_binary_loss + task_loss_org + pseudo + mlm_loss #+ task_loss_cotrain*task_loss_cotrain/(task_loss_cotrain+task_loss_org)
