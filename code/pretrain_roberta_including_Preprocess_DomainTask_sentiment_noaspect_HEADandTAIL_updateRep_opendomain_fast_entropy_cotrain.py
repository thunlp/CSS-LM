    ...
    if args.do_train:
        ...
        for epo in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch_ in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                #task_loss_cotrain, class_logit_cotrain, out_domain_rep_head, out_domain_rep_tail = model(input_ids_org=input_ids_org, sentence_label=sentiment_label, attention_mask=input_mask, func="task_class", from_query=True)
                task_loss_cotrain, class_logit_cotrain, out_domain_rep_head, out_domain_rep_tail = model(input_ids_org=input_ids_org, sentence_label=sentiment_label, attention_mask=input_mask, func="task_class", from_query=True)
                ###
                ...
                if n_gpu > 1:
                    ...
                    loss = domain_binary_loss.mean() + task_binary_loss.mean() + task_loss_org.mean() + pseudo + task_loss_cotrain.mean()*(task_loss_cotrain.mean()/(task_loss_cotrain.mean()+task_loss_org.mean())) + mlm_loss.mean()
                    ...
                    loss = domain_binary_loss + task_binary_loss + task_loss_org + pseudo + task_loss_cotrain*(task_loss_cotrain/(task_loss_cotrain+task_loss_org)) + mlm_loss
