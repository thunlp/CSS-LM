    ...
    if args.do_train:
        ...
        for epo in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch_ in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                in_domain_binary_loss, domain_binary_logit = model(func="domain_binary_classifier", in_domain_rep=comb_rep_pos.to(device), out_domain_rep=comb_rep_unknow.to(device), domain_id=domain_id,use_detach=False)
                ############
                ...
                if n_gpu > 1:
                    ...
                    #loss = task_loss_org.mean() + in_domain_task_binary_loss.mean()  + in_domain_binary_loss.mean() + in_task_binary_loss.mean()
                    loss = task_loss_org.mean() + in_domain_binary_loss.mean() + in_task_binary_loss.mean()
