...
from transformers.modeling_roberta_updateRep_self import RobertaForMaskedLMDomainTask, RobertaForSequenceClassification
...
def load_data():
    with open(file_in) as f:
        ...
        counter_more_than_50_words=0
        for index, d in tqdm(enumerate(f)):
            if counter_more_than_50_words >= int(num_samples):
                break
            #if len(d) == 0:
            if len(d) <= 50:
                continue
            else:
                counter_more_than_50_words+=1
                print(float(counter_more_than_50_words/int(num_samples))*100,"%")
