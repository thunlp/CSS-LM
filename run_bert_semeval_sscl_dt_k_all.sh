...
do
    ...
    #python3 extract_instance.py train_all.json 16
    ...
    OUTFILE="output_pretrain_bert_including_Preprocess_DomainTask_sentiment_noaspect_HEADandTAIL_opendomain_entropy_sscl_dt_all"
    ###
    rm -rf $OUTFILE
    mkdir $OUTFILE
    ...
    DATA_out=data/opendomain_finetune_noword_10000_all/

    #fix
    ###
    #CUDA_VISIBLE_DEVICES=$TRAIN_GPU python3.6 -W ignore::UserWarning code/init_sscl_dt.py  --num_labels_task $NUM_LABEL --do_train   --do_lower_case   --data_dir_outdomain $DATA_out  --data_dir_indomain $DATA_in --pretrain_model $MODEL --max_seq_length $MAX_LENGTH --train_batch_size $BATCH_SIZE --learning_rate $LEARNING_RATE   --num_train_epochs $N_times_1   --output_dir $OUTFILE  --loss_scale 128 --weight_decay 0 --adam_epsilon 1e-8 --max_grad_norm 1 --fp16_opt_level O1 --task 0 --fp16 --augment_times 20 --K 16
    ...
    OUTPUT=data/opendomain_finetune_noword_10000_all
    #fix
    ###
    MODEL=bert-base-768-yelp-DomainTask-noword-sentiment-HEADandTAIL-opendomain-entropy-self_all
    ###
    mkdir $OUTPUT
    mkdir $MODEL
    rm -rf $MODEL/*
    ...
    ######################################################
    ######################################################
    mkdir backup_semeval_all
    rm backup_semeval_all/*
    cp -r $OUTFILE/* backup_semeval_all/
    ...
    do
        ...
        rm -rf $OUTFILE
        mkdir $OUTFILE
        cp -r backup_semeval_all/* $OUTFILE/
        ##
        ###
        INPUT=data/openwebtext/train.txt
        OUTPUT=data/opendomain_finetune_noword_10000_all
